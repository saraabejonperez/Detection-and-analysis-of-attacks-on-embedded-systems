...
PATH_TONIOT = PATH_DATA / 'toniot_train.csv'
...
def save_preprocessing_artifacts(feature_names: pd.Index, output_path: Path, model: RandomForestClassifier, suffix: str) -> None:
    """
    ...
    """
    try:
        np.save(output_path / f'features_{suffix}.npy', feature_names.to_numpy())
        joblib.dump(model, output_path / f'dos_classifier_{suffix}.pkl')
...
def plot_confusion_matrix(y_true: Iterable[int], y_pred: Iterable[int], 
    ...
    :type class_names: Tuple(str, str)
...
def run_training_pipeline(traffic_df: pd.DataFrame, suffix: str) -> None:
    """
    This function splits the data into training and test sets, balances and scales the training data,
    trains and evaluates the neural network model, generates evaluation metrics and confusion matrices
    and finally exports the trained model.

    :param traffic_df: DataFrame with all the data for training and validating the model
    :type traffic_df: pd.DataFrame
    :param suffix: Suffix for the name of saved files
    :type suffix: String
    """
    ...
    plot_confusion_matrix(y_test, y_pred, class_names=('Benign', 'DoS'), save_path=PATH_MODEL / f'confusion_matrix_{suffix}.png')
    plot_confusion_matrix(y_test, y_pred, class_names=('Benign', 'DoS'), normalize=True, save_path=PATH_MODEL / f'confusion_matrix_normalized_{suffix}.png')
    ...
    save_preprocessing_artifacts(feature_names=feature_names, output_path=PATH_MODEL, model=final_model, suffix=suffix)